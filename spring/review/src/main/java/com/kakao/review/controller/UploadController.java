...
public class UploadController {
    @Value("${com.gitshineit.upload.path}")
    ...
    public ResponseEntity<List<UploadResultDTO>> uploadFile(MultipartFile[] uploadFiles){
        ...
        for(MultipartFile uploadFile : uploadFiles){
            if(uploadFile.getContentType().startsWith("image") == false){
                log.warn("이미지 파일이 아님");
                return new ResponseEntity<>(HttpStatus.FORBIDDEN); //403 ERROE
            }
            //업로드 된 파일의 파일 이름
            String originalName = uploadFile.getOriginalFilename();
            //IE는 파일 이름이 아니고 전체 경로를 전송하기 때문에 마지막 \ 뒤 부분만 추출
            ...
            try {
                ...
                //썸네일 파일 이름 생성
                File thumbnameFile = new File(uploadPath +
                        File.separator + realUploadPath + File.separator + "s_" + uuid + fileName);
                //썸네일 생성
                Thumbnailator.createThumbnail(saveFile, thumbnameFile, 100, 100);
                //결과를 list에 추가
